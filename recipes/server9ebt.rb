...
filename = File.basename(node['sqlanywhere']['server9ebt']['url']).downcase
fileextension = File.extname(filename)
download_path = "#{Chef::Config['file_cache_path']}/#{filename}"
extract_path = "#{Chef::Config['file_cache_path']}/#{node['sqlanywhere']['server9ebt']['filename']}/#{node['sqlanywhere']['server9ebt']['checksum']}"
winfriendly_extract_path = win_friendly_path(extract_path)
config_file_path = "#{extract_path}/setup.iss"

remote_file download_path do
  source node['sqlanywhere']['server9ebt']['url']
  checksum node['sqlanywhere']['server9ebt']['checksum']
end
...
template config_file_path do
  source 'server9ebt.iss.erb'
end

windows_package node['sqlanywhere']['server9ebt']['name'] do
